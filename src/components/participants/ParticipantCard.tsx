import { useTranslation } from 'react-i18next'
import type { Participant } from '../../types'
import { Badge } from '../ui/Badge'
import { Button } from '../ui/Button'
import { generateOsmLink } from '../../lib/coordinates'

interface ParticipantCardProps {
  participant: Participant
  onEdit: () => void
  onDelete: () => void
}

export function ParticipantCard({ participant, onEdit, onDelete }: ParticipantCardProps) {
  const { t } = useTranslation()

  const preferenceVariant = participant.preference
    ? (participant.preference as 'starter' | 'main' | 'dessert' | 'prefer-not')
    : null

  const preferenceLabel = participant.preference
    ? t(`preferences.${participant.preference === 'prefer-not' ? 'preferNot' : participant.preference}`)
    : t('preferences.noPreference')

  return (
    <div className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
      <div className="flex w-full min-w-0 items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-semibold text-gray-900 dark:text-gray-100">
            {participant.name}
          </h3>
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            <Badge variant="count">
              {participant.count === 1 ? t('count.one') : t('count.two')}
            </Badge>
            {preferenceVariant ? (
              <Badge variant={preferenceVariant}>{preferenceLabel}</Badge>
            ) : (
              <Badge variant="neutral">{preferenceLabel}</Badge>
            )}
          </div>
          <div className="mt-2 flex items-center gap-1">
            <p className="truncate text-sm text-gray-600 dark:text-gray-400">
              {participant.address}
            </p>
            {participant.coordinates && (
              <a
                href={generateOsmLink(participant.coordinates)}
                target="_blank"
                rel="noopener noreferrer"
                title={t('common.openMap')}
                className="shrink-0 text-blue-500 hover:text-blue-700 dark:text-blue-400"
              >
                <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                  <path
                    fillRule="evenodd"
                    d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
            )}
          </div>
        </div>
        <div className="flex shrink-0 gap-1">
          <Button
            variant="ghost"
            size="sm"
            onClick={onEdit}
            aria-label={t('participants.edit')}
            className="p-1.5"
          >
            <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path d="M2.695 14.763l-1.262 3.154a.5.5 0 00.65.65l3.155-1.262a4 4 0 001.343-.885L17.5 5.5a2.121 2.121 0 00-3-3L3.58 13.42a4 4 0 00-.885 1.343z" />
            </svg>
          </Button>
          <Button
            variant="ghost"
            size="sm"
            onClick={onDelete}
            aria-label={t('participants.delete')}
            className="p-1.5 text-red-500 hover:text-red-700 dark:text-red-400"
          >
            <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path
                fillRule="evenodd"
                d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z"
                clipRule="evenodd"
              />
            </svg>
          </Button>
        </div>
      </div>
    </div>
  )
}
